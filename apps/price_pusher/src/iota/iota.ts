...
import { IotaClient, IotaObjectRef, PaginatedCoins } from "@iota/iota-sdk/client";
...
type IotaAddress = string;

export class IotaPriceListener extends ChainPriceListener {
  private pythClient: IotaPythClient;
  private provider: IotaClient;
  ...
  constructor(
    ...
    this.provider = new IotaClient({ url: endpoint });
  ...
  async getOnChainPriceInfo(priceId: string): Promise<PriceInfo | undefined> {
    try {
      ...
      this.logger.error(
        err,
        `Polling IOTA on-chain price for ${priceId} failed.`,
      );
      return undefined;
    }
  }
...
/**
 * The `IotaPricePusher` is designed for high-throughput of price updates.
 ...
 * jumping through some IOTA-specific hoops in order to maximize parallelism.
 *
 ...
 */
export class IotaPricePusher implements IPricePusher {
  constructor(
    private readonly signer: Ed25519Keypair,
    private readonly provider: IotaClient,
    private logger: Logger,
    private hermesClient: HermesClient,
    private gasBudget: number,
    private gasPool: IotaObjectRef[],
    private pythClient: IotaPythClient,
  ) {}
  ...
  static async createWithAutomaticGasPool(
    ...
    const provider = new IotaClient({ url: endpoint });

    const gasPool = await IotaPricePusher.initializeGasPool(
      keypair,
      provider,
      numGasObjects,
      ignoreGasObjects,
  ...
  private async sendTransactionBlock(tx: Transaction): Promise<void> {
    ...
    let nextGasObject: IotaObjectRef | undefined = undefined;
    try {
      ...
      if (
        ...
        nextGasObject = await IotaPricePusher.tryRefreshObjectReference(
          this.provider,
          gasObject,
        );
      }
  ...
  private static async initializeGasPool(
    ...
    const signerAddress = signer.toIotaAddress();
    ...
    const consolidatedCoin = await IotaPricePusher.mergeGasCoinsIntoOne(
      signer,
      provider,
      signerAddress,
      ignoreGasObjects,
    ...
    const gasPool = await IotaPricePusher.splitGasCoinEqually(
      signer,
      provider,
      signerAddress,
      Number(splitAmount),
  ...
  private static async splitGasCoinEqually(
    signer: Ed25519Keypair,
    provider: IotaClient,
    signerAddress: IotaAddress,
    splitAmount: number,
    numGasObjects: number,
    gasCoin: IotaObjectRef,
  ): Promise<IotaObjectRef[]> {
  ...
  private static async mergeGasCoinsIntoOne(
    ...
    const gasCoins = await IotaPricePusher.getAllGasCoins(provider, owner);
    // skip merging if there is only one coin
    if (gasCoins.length === 1) {
      return gasCoins[0];
    }

    const gasCoinsChunks = chunkArray<IotaObjectRef>(
      gasCoins,
    ...
    return finalCoin as IotaObjectRef;
  }
}
